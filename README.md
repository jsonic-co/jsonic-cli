...
**This project is now archived. If you want to use Jsonic as a CLI client for CI/CD purposes, please use the new [Jsonic CLI](https://docs.jsonic.ir/cli).**

⚠️⚠️⚠️⚠️⚠️⚠️

# Jsonic CLI [![jsonic](https://img.shields.io/badge/Made_for-Jsonic-hex_color_code?logo=Jsonic)](https://jsonic.ir) [![Go Report Card](https://goreportcard.com/badge/github.com/athul/pwcli)](https://goreportcard.com/report/github.com/athul/pwcli)
...
```shell
$ git clone https://github.com/jsonic-co/jsonic-cli.git
```
...
- You can download prebuilt binaries from the [Releases](https://github.com/jsonic-co/jsonic-cli/releases) page.
- **Supported platforms**:
  - Linux (x64, x86)
  - Mac (x64)
  - Windows (x64, x86)
...
------------- | ------------------------------------------------- | -----------------------------
jsonic-cli-bin  | https://aur.archlinux.org/packages/jsonic-cli-bin/  | Pre-built binary
jsonic-cli      | https://aur.archlinux.org/packages/jsonic-cli/      | Compiled from latest release
jsonic-cli-git  | https://aur.archlinux.org/packages/jsonic-cli-git/  | Compiled from latest commit
...
```shell
brew install athul/tap/jsonic-cli
```

### Windows

You can download pre-built binaries from the [Releases](https://github.com/jsonic-co/jsonic-cli/releases) page.

Alternatively, you can install `jsonic-cli` via [Scoop](https://scoop.sh/):

```shell
scoop install jsonic-cli
```
...
- GET : `$ jsonic-cli get <url>`
- POST: `$ jsonic-cli post <url>`
- PATCH: `$ jsonic-cli patch <url>`
- PUT : `$ jsonic-cli put <url>`
- DELETE: `$ jsonic-cli delete <url>`

Example for a POST request:

```shell
$ jsonic-cli post https://reqres.in/api/users/2 -c js -b '{"name": "morp","job": "zion resident"}'
```
...
This can be used to test multiple endpoints from the `jsonic-collection.json` file.
...
```shell
$ jsonic-cli send <PATH to jsonic-collection.json>
```
...
The `gen` command generates the API documentation from `jsonic-collection.json` file and serves it as a static page on port `1341`.

Example:

```shell
$ jsonic-cli gen <PATH to jsonic-collection.json>
```

Sample Hosted site: https://jsonic-docsify.surge.sh/
...
```shell
$ jsonic-cli get -H 'X-Api-Key: foobar' -H 'X-Api-Secret: super_secret' https://example.com/api/v1/accounts
```
...
```shell
$ echo '{"foo":"bar"}' | jsonic-cli post -c js http://example.com
```
...
$ jsonic-cli post -c js http://example.com <myrequest.json
```
...
```shell
$ jsonic-cli post https://reqres.in/api/users/2 -c js -e
```
